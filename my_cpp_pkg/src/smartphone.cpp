    ...
    public:
        SmartPhoneNode() : Node("node_name") {
            subscriber_ = this->create_subscription<example_interfaces::msg::String>("robot_news", 
                                        10, std::bind(&SmartPhoneNode::callbackRobotNews, 
                                        this, std::placeholders::_1)); // Each parameter in the
                                        // callback, so if there are two paramateres the placeholder
                                        // value will be equal to _2!!!
